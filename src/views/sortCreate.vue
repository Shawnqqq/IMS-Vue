  ...
  <div class="sort-insert-section">
    ...
    <form class="sort-insert-form">
      <div class="sort-input">
        ...
        <input :class="{active:nameRule}" type="text" placeholder="请输入名称" v-model="value.name">
        <p class="sort-input-name-message" :class="{active:nameRule}">名称不能为空</p>
      </div>
    </form>
    <button class="sort-insert-btn" @click="handleCreate()">提交</button>
  </div>
</template>
...
export default {
  ...
  methods:{
    handleCreate(){
      ...
      if(islock){
        sortService.insert(params).then(res =>{
          if(res.code === 200){
            alert(res.message)
